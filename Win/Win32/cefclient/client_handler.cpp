...
enum client_menu_ids {
  ...
  CLIENT_ID_OPEN_FILEDIALOG,
  CLIENT_ID_SAVE_FILEDIALOG,
...
ClientHandler::ClientHandler()
  ...
  if (startup_url_.empty())
    startup_url_ = "file:///C:/Users/t_chenb/Desktop/JQuery/jquery.jqplot.1.0.4r1120/dist/examples/barTestbk.html";
...
void ClientHandler::OnBeforeContextMenu(
  ...
  if ((params->GetTypeFlags() & (CM_TYPEFLAG_PAGE | CM_TYPEFLAG_FRAME)) != 0) {
	...
	model->AddItem(CLIENT_ID_OPEN_FILEDIALOG, "Select File From");
	model->AddItem(CLIENT_ID_SAVE_FILEDIALOG, "Save File To");
...
bool ClientHandler::OnContextMenuCommand(
  ...
  switch (command_id) {
    ...
    case CLIENT_ID_CLOSE_DEVTOOLS:
      ...
      return true;
	case CLIENT_ID_OPEN_FILEDIALOG:
		OnFileDialog(browser,FILE_DIALOG_OPEN,"Open File Dialog","SelectFile",accept_types,0);
		return true;
	case CLIENT_ID_SAVE_FILEDIALOG:
		OnFileDialog(browser,FILE_DIALOG_SAVE,"Save File Dialog","SaveFile",accept_types,0);
		return true;
...
bool ClientHandler::OnFileDialog(CefRefPtr<CefBrowser> browser,
  ...
  TCHAR fliter[256]=TEXT("TEXT(*.txt)\0*.txt\0Cpp(*.c)\0*.c\0All\0*.*\0");
  ...
  ofn.nFilterIndex =1;
  lstrcpy(szPathname, default_file_name.c_str());
  ofn.lpstrFile = szPathname;
  ofn.nMaxFile = _countof(szPathname);
  ofn.lpstrTitle = title.c_str();
  ofn.lpstrInitialDir=NULL ; 
  if(FILE_DIALOG_OPEN==mode)
  {
